...
program
  .name('pocketvex')
  .description('Schema migration system for PocketBase with real-time development workflow')
  .version(version);
...
program
  ...
  .action(() => {
    console.log(chalk.blue('🚀 PocketVex - Schema Migration System for PocketBase\n'));
    ...
    console.log(chalk.gray('  3. Edit schema files and see changes applied automatically\n'));
    
    console.log(chalk.yellow('Commands:'));
    console.log(chalk.gray('  dev --watch     Start development server with file watching'));
    console.log(chalk.gray('  schema apply    Apply schema changes to PocketBase'));
    console.log(chalk.gray('  schema diff     Show schema differences'));
    console.log(chalk.gray('  migrate generate Generate migration from schema changes'));
    console.log(chalk.gray('  migrate up      Run pending migrations'));
    console.log(chalk.gray('  migrate down    Rollback last migration'));
    console.log(chalk.gray('  status          Show PocketBase connection status\n'));
    
    console.log(chalk.yellow('Environment Variables:'));
    console.log(chalk.gray('  PB_URL          PocketBase server URL (default: http://127.0.0.1:8090)'));
