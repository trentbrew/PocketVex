...
$jobs.register('hourly-analytics', '0 0 * * * *', async (cron) => {
  ...
  try {
    ...
    console.log(
      `✅ Analytics generated: ${pageViews} views, ${newUsers} new users`,
    );
  } catch (error) {
...
$jobs.register('email-processor', '0 */5 * * * *', async (cron) => {
  ...
  try {
    const emailQueue = await $app
      .db()
      .newQuery('email_queue')
  ...
  try {
    ...
    const weeklyStats = {
      newUsers: await $app
        ...
        .count(),
      totalPosts: await $app
        .db()
        .newQuery('posts')
        .filter('created >= {:weekStart}', { weekStart })
        .count(),
      totalViews: await $app
        .db()
        .newQuery('page_views')
        .filter('created >= {:weekStart}', { weekStart })
        .count(),
      totalEmails: await $app
        .db()
        .newQuery('email_queue')
        .filter('created >= {:weekStart}', { weekStart })
        .count(),
...
async function processDataExportTask(task) {
  ...
  const records = await $app
    .db()
    .newQuery(exportData.collection)
    .filter(exportData.filter || '')
    .all();

  console.log(
    `Exporting ${records.length} records from ${exportData.collection}`,
  );
...
async function processCleanupTask(task) {
  ...
  if (cleanupData.collection && cleanupData.olderThan) {
    ...
    const oldRecords = await $app
      .db()
      .newQuery(cleanupData.collection)
    ...
    console.log(
      `Cleaned up ${oldRecords.length} old records from ${cleanupData.collection}`,
    );
  }
}
