...
$jobs.register({
  ...
  handler: async (e) => {
    ...
    try {
      ...
      const oldSessions = await $app.db().newQuery(`
...
$jobs.register({
  ...
  handler: async (e) => {
    ...
    try {
      ...
      const newUsers = await $app.db().newQuery(`
      ...
      const newPosts = await $app.db().newQuery(`
        SELECT COUNT(*) as count 
        FROM posts 
        WHERE created > datetime('now', '-7 days')
      `).one();
      ...
      if (response.status === 200) {
        ...
        for (const item of data.items) {
          await $app.db().newQuery(`
            ...
            VALUES (?, ?, ?, datetime('now'))
          `).bind(item.id, item.name, item.value).execute();
        }
...
$jobs.register({
  ...
  handler: async (e) => {
    ...
    try {
      ...
      const popularPosts = await $app.db().newQuery(`
        SELECT * FROM posts 
