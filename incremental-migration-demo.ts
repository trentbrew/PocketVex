        ...
        },
        {
          ...
          schema: [
            ...
            { name: 'created_at', type: 'date', required: true },
            {
              ...
              type: 'select',
              options: {
                values: ['tech', 'life', 'work', 'tutorial'],
                maxSelect: 5,
              },
            },
...
async function runIncrementalDemo() {
  ...
  try {
    ...
    schemaVersions.forEach((version, index) => {
      ...
      console.log(
        chalk.gray(
          `  ${index + 1}. ${version.name} (${collectionCount} collections)`,
        ),
      );
    });
...
async function applyIncrementalMigration(
  ...
  console.log(
    chalk.blue('🎉 Your PocketBase schema has been updated in real-time!'),
  );
  console.log(
    chalk.gray(
      'Visit your admin panel to see the changes: https://pocketvex.pockethost.io/_/',
    ),
  );
  ...
  if (plan.unsafe.length > 0) {
    ...
    console.log(
      chalk.gray(
        'Run "pocketvex migrate generate" to create migration files for review.',
      ),
    );
  }
}
