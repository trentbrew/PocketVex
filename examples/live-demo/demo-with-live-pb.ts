...
async function runDemo() {
  ...
  console.log(
    chalk.gray('Live PocketBase URL: https://pocketvex.pockethost.io/'),
  );
  console.log(chalk.gray('Admin Email: ' + LIVE_PB_CONFIG.adminEmail));
  console.log(
    chalk.gray(
      'Admin Password: ' + (LIVE_PB_CONFIG.adminPassword ? '***' : 'Not set'),
    ),
  );
  ...
  try {
    ...
    if (isConnected) {
      ...
      if (currentSchema.collections.length > 0) {
        ...
        console.log(
          chalk.blue(
            '🎉 Fresh PocketBase instance - ready for schema migration!',
          ),
        );
      }
      ...
      console.log(
        chalk.gray(
          '  1. Get the correct admin credentials from your PocketBase instance',
        ),
      );
    ...
    console.log(
      chalk.gray(
        'Even without a connection, you can see the schema comparison logic in action.',
      ),
    );
  }
  ...
  console.log(
    chalk.blue('  pocketvex dev --watch     # Start development server'),
  );
  ...
  console.log(
    chalk.blue(
      '📖 This demonstrates how PocketVex integrates with live PocketBase instances.',
    ),
  );
}
