...
const LIVE_CONFIG = {
  ...
  adminEmail: 'demo@example.com', // Update with actual credentials
  adminPassword: 'myPassword123', // Update with actual password
};

console.log(
  chalk.blue('🚀 PocketVex Live Demo with Real PocketBase Instance\n'),
);
...
async function runLiveDemo() {
  ...
  try {
    ...
    if (!isConnected) {
      ...
      console.log(
        chalk.gray('  - URL is correct: https://pocketvex.pockethost.io/'),
      );
    ...
    const currentSchemaSpinner = ora(
      'Fetching current schema from PocketBase...',
    ).start();
    ...
    if (plan.safe.length > 0) {
      console.log(
        chalk.green('✅ Safe changes would be applied automatically:'),
      );
    ...
    if (plan.safe.length > 0) {
      ...
      console.log(
        chalk.gray('In a real scenario, these would be applied automatically:'),
      );

      for (const operation of plan.safe.slice(0, 3)) {
      ...
      if (plan.safe.length > 3) {
        console.log(
          chalk.gray(`... and ${plan.safe.length - 3} more operations`),
        );
      }
    }
    ...
    if (plan.unsafe.length > 0) {
      ...
      console.log(
        chalk.gray('Unsafe changes would generate migration files like:'),
      );

      const migrationExample = `// Migration: ${
        new Date().toISOString().split('T')[0]
      }_${plan.unsafe.length}_unsafe_changes.js
...
export const down = async (pb) => {
  // Rollback implementation
    ...
    console.log(
      chalk.blue('  export PB_URL="https://pocketvex.pockethost.io/"'),
    );
    ...
    console.log(
      chalk.blue(
        '📖 This demonstrates real-time schema migration with a live PocketBase instance.',
      ),
    );
  } catch (error) {
